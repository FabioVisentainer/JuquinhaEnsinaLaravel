  ...
  <body class="body bg-slate-50">
      <nav class="flex justify-between items-center">
          <div class="p-5 md:flex md:justify-between max-[770px]:hidden">
              <div class="max-[770px]:hidden">
                  ...
                  <a href="{{ route('coordinators.notifications.home') }}">
                      <img class="invertColor cursor-pointer pointer-events-auto" src="{{ asset('images/voltar.png') }}" alt="Back button" width="40px">
                  </a>
              </div>
          </div>
          ...
          <div class="flex gap-8 p-5">
              ...
              <div>
                  <a href="{{ route('coordinators.notifications.home') }}">
                      <img id="notificationIcon" class="invertColor cursor-pointer" src="{{ asset('images/notificacao.png') }}" alt="Notification button" width="40px">
                  </a>
              </div>
      ...
      <div class="total relative top-1 transition-all duration-300 ease-in-out flex justify-center items-center min-h-full p-4">
          <div class="w-full max-w-lg bg-gray-200 p-6 rounded-lg shadow-lg dark:bg-gray-800 border border-gray-300">
              <h1 class="text-3xl font-bold text-center mb-4 dark:text-white">Notificação Para {{ $class->class_name }} - {{ $class->class_year }}</h1>
              ...
              <form action="{{ route('coordinators.notifications.store.class', ['class_id' => $class->class_id]) }}" method="POST">
                  @csrf
