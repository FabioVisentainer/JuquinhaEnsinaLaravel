...
<div class="total relative top-1 transition-all duration-300 ease-in-out">
  <h1 class="pageTitle text-4xl md:text-6xl">Cadastro de Cronogramas</h1>

  @if(session('success'))
      <div class="alert alert-success mt-4 text-lg text-green-600 font-semibold dark:text-green-400">
          {{ session('success') }}
      </div>
  @endif

  <a href="{{ route('coordinators.registries.chronograms.new') }}">
      <div class="flex justify-center items-center mt-4">
          <button class="blueButton rounded-xl sm:w-[50%] md:w-[35%] lg:w-[20%]">Novo Cronograma</button>
      </div>
  </a>

  @if($chronograms->isEmpty())
      <p class="mt-8 text-2xl text-gray-700 dark:text-gray-300">Nenhum Cronograma encontrado para sua entidade.</p>
  @else
          ...
          @foreach($chronograms as $chronogram)
                <div class="class bg-white p-4 rounded-lg shadow flex flex-col md:flex-row md:justify-between items-start md:items-center dark:bg-gray-700">
                    <div class="flex flex-col">
                        <h3 class="text-3xl font-semibold dark:text-white">{{ $chronogram->chronogram_name }}</h3>
                        <p class="mt-1 text-gray-600 text-xl dark:text-gray-300"><span class="font-bold">Ativo?</span> {{ $chronogram->is_active ? 'Sim' : 'Não' }}</p>
                        <p class="mt-1 text-gray-600 text-xl dark:text-gray-300"><span class="font-bold">Data de Criação:</span> {{ $chronogram->created_at }}</p>
                    </div>
                    <a href="{{ route('coordinators.registries.chronograms.edit', $chronogram->chronogram_id ) }}" class="mt-4 md:mt-0">
