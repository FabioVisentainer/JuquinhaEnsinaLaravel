  ...
  <body class="body bg-slate-50">
      ...
      <div class="total relative top-1 transition-all duration-300 ease-in-out flex justify-center items-center min-h-full p-4">
          <div class="w-full max-w-lg bg-gray-200 p-6 rounded-lg shadow-lg dark:bg-gray-800 border border-gray-300">
              <h1 class="text-3xl font-bold text-center mb-4 dark:text-white">Registrar Novo Cronograma</h1>

              <form action="{{ route('coordinators.registries.chronograms.store') }}" method="POST">
                  @csrf

                  <!-- Incluir o componente de formulário de cronograma -->
                  <x-registries.chronogram_form />

                  <div class="flex justify-center mt-4">
                      <button type="submit" class="btn bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-300">Registrar Cronograma</button>
                  </div>
              </form>
          </div>
      </div>
