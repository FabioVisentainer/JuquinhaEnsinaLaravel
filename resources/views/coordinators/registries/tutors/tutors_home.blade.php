<x-layout>
  <body class="body bg-slate-50">
      ...
      <div class="total relative top-1 transition-all duration-300 ease-in-out">
          <h1 class="pageTitle text-4xl md:text-6xl">Cadastro de Responsáveis</h1>
          @if(session('success'))
          ...
          @if($tutors->isEmpty())
              <h1 class="pageTitle text-2xl md:text-3xl">Não há Responsáveis cadastrados na sua entidade</h1>
          @else
              <div class="container mx-auto p-4 py-8">
                  ...
                  <div class="flex justify-center items-center">
                      <button class="blueButton rounded-xl sm:w-[50%] md:w-[35%] lg:w-[30%] "><a href="{{ route('coordinators.registries.tutors.new') }}">Cadastrar novo Responsável</a>
                      </button>
                  </div>
                      ...
                      @foreach($tutors as $tutor)
                          <div class="tutor bg-white p-4 rounded-lg shadow flex justify-between items-center dark:bg-gray-700">
                              <div class="flex flex-col">
                                  ...
                                  <p class="mt-1 ml-0.5 text-gray-600 text-xl dark:text-gray-300"><span class="font-bold">Status:</span> {{ $tutor->is_active ? 'Ativo' : 'Inativo' }}</p>
                                  <p class="text-gray-600 md:hidden dark:text-gray-300"><span class="font-bold ml-0.5">Data de Registro:</span> {{ $tutor->tutor_registry_date }}</p>
                                  <a href="{{ route('coordinators.registries.tutors.edit', $tutor->tutor_id) }}"><button class="mt-4 w-36 flex items-center justify-center bg-blue-500 text-white rounded-xl hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 py-3 mb-3">Editar</button></a> 
                              </div>
                              <div class="hidden md:flex items-center text-gray-600 dark:text-gray-300">
                                  <p><span class="font-bold">Data de Registro:</span> {{ $tutor->tutor_registry_date }}</p>
                              </div>
                          </div>
                      @endforeach
                  </div>
