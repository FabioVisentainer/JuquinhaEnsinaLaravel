<x-layout>
  <body class="body bg-slate-50">
    ...
    <div class="total relative top-1 transition-all duration-300 ease-in-out">
      <h1 class="pageTitle text-4xl md:text-6xl">Cadastro de Turmas</h1>
      <p class="text-xl text-gray-700 dark:text-gray-300 mt-2 text-center font-londrina">
        Atenção: Para salvar alterações de disciplinas e assuntos aos alunos, é necessário adicioná-los à turma somente depois de ter editado a disciplina/turma.
      </p>
      ...
      <a href="{{ route('coordinators.registries.classes.new') }}">
        <div class="flex justify-center items-center">
          <button class="blueButton rounded-xl sm:w-[50%] md:w-[35%] lg:w-[20%] -mb-8">Cadastrar nova Turma</button>
        </div>
      </a>

      @if($classes->isEmpty())
        <p class="mt-8 text-2xl text-gray-700 dark:text-gray-300">Sem turmas encontradas para sua entidade.</p>
      @else
          ...
          <div class="grid gap-4 bg-gray-200 rounded-lg w-full dark:bg-gray-800 p-4">
            @foreach($classes as $class)
              <div class="class bg-white p-4 rounded-lg shadow flex flex-col md:flex-row md:justify-between items-start md:items-center dark:bg-gray-700">
                ...
                <a href="{{ route('coordinators.registries.classes.edit', $class->class_id ) }}" class="mt-4 md:mt-0">
