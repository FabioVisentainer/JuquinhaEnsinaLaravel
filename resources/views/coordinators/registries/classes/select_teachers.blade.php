      ...
      <nav class="flex justify-between items-center">
        <div class="p-5 md:flex md:justify-between max-[770px]:hidden">
          <div class="max-[770px]:hidden">
            ...
            <a href="{{ route('coordinators.registries.classes.edit', $class->class_id) }}">
        ...
        <div>
          ...
          <img class="x invertColor cursor-pointer hidden transition-all ease-linear duration-500 md:hidden max-[800px]:ml-3" src="{{ asset('images/x.png') }}" alt="Sair do menu hamburguer" width="30px">
        </div>
        ...
        <div class="total relative top-1 transition-all duration-300 ease-in-out">
          <div>
            <h1 class="pageTitle text-4xl md:text-6xl mt-4 text-gray-800 dark:text-white">
              Selecionar Professor para Turma: {{ $class->class_name }} - {{ $class->class_year }}
            </h1>
          </div>
          ...
          <div class="container mx-auto p-4 py-8">
            <form action="{{ route('coordinators.save_teachers', $class->class_id) }}" method="POST" class="space-y-4">
              @csrf
  
              <div id="teacherList" class="grid gap-4 p-4 bg-gray-200 rounded-lg w-full dark:bg-gray-800 mt-6">
                <div class="teacher bg-white p-4 rounded-lg shadow flex flex-col dark:bg-gray-700">
                  <!-- Contêiner da Tabela -->
                      ...
                      <tbody>
                        @foreach($teachers as $teacher)
                          <tr class="border-b border-gray-300 dark:border-gray-600">
                            ...
                            <td class="p-2 text-gray-700 dark:text-gray-300">{{ $teacher->teacher_name }}</td>
                          </tr>
                        @endforeach
                      </tbody>
                    </table>
              ...
              <div class="flex justify-center mt-6">
                <button 
                  ...
                  Atualizar Associação de Professores
                </button>
              </div>
            </form>
          </div>
